...
inline void gasal_kernel_launcher_local_tb(int32_t N_BLOCKS, int32_t BLOCKDIM, algo_type algo, comp_start start, gasal_gpu_storage_t *gpu_storage, int32_t actual_n_alns, int32_t k_band, data_source semiglobal_skipping_head, data_source semiglobal_skipping_tail, Bool secondBest, uint32_t maximum_sequence_length, short2* global_inter_row, uint32_t* global_direction, uint8_t *result_query, uint8_t *result_target, short2 *dblock_row, short2 *dblock_col)
{
	switch(algo)
	{
		KERNEL_SWITCH_LOCAL(LOCAL,		start, semiglobal_skipping_head, semiglobal_skipping_tail, secondBest, maximum_sequence_length, global_inter_row);
		default:
		break;
